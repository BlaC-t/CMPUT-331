...
def getLetterFrequency(message):
    ...
    for i in letterCount:
        letterCount[i] = (letterCount[i] / len(message))
...
def vigenereKeySolver(ciphertext: str, keylength: int):
    ...
    for eachSequence in subSequences:
        possKeyList.append(calculateTopIMC(eachSequence))
...
def hackVigenere(ciphertext: str):
    ...
    possKeyLen = a7.keyLengthIC(ciphertext, 15)
    ...
    possKeyList = []
    for eachSet in totalKeyList:
        for eachWord in eachSet:
            score = keyScore(eachWord, ciphertext, freqDict, nGram)
            possKeyList.append((eachWord, score))
...
def test():
    ...
    crackPassword()
    print("Running time: ", time.time() - startingTime)
