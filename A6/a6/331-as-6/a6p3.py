...
def bestSuccessor(mapping: dict, ciphertext: str, frequencies: dict, n: int) -> dict:
    ...
    cmb = list(itertools.combinations(ascii_uppercase, 2))

    for i in cmb:
        mapA = mapping.copy()

        mapA[i[0]], mapA[i[1]] = mapping[i[1]], mapping[i[0]]

        bestScore = keyScore(bestMapping, ciphertext, frequencies, n)
        score = keyScore(mapA, ciphertext, frequencies, n)

        if score == bestScore:
            bestMapping = breakKeyScoreTie(mapping, bestMapping, mapA)
        elif score > bestScore:
            bestMapping = mapA.copy()
        else:
            continue
...
def breakKeyScoreTie(originalMapping, successorMappingA, successorMappingB):
    ...
    aSwapped = "".join(
        sorted(
            k
            for k, v in (set(successorMappingA.items()) - set(originalMapping.items()))
        )
    )
    bSwapped = "".join(
        sorted(
            k
            for k, v in (set(successorMappingB.items()) - set(originalMapping.items()))
        )
    )
